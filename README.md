...
$$
\frac{\partial \hat{C}_A}{\partial \hat{t}} + r_v\,\frac{\partial \hat{C}_A}{\partial \hat{x}}
= \frac{1}{\mathrm{Pe}_A}\,\frac{\partial^2 \hat{C}_A}{\partial \hat{x}^2}
- \mathrm{Da}_A\,(\hat{C}_A - \hat{W})
$$
...
- MODELO 3 : Tercera etapa para llegar a la simulación final. 
    ...
    Se resuelve por tanto el sistema:
     ...
     En $x=0$: $\hspace{1cm} \hat{C}_A (0) = \hat{C}_{A,in}$, $\hat{W}(0) = \hat{W}_{in}$  (Dirichlet en la entrada)
  
     En $x=1$: $\hspace{1cm} \frac{d \hat{C}_A}{dx} = 0$, $\frac{d \hat{W}}{dx} = 0$ (Neumann en la salida)
